...
import mySettings
# style_images = glob.glob('styles/*.jpg') # 返回styles文件夹下的所有.jpg文件  --->列表
style_image = 'styles/%s.jpg'%mySettings.style_name # 返回styles文件夹下的所有.jpg文件  --->列表
# print(style_images)
...
X_data = []
image_size = 256
paths = glob.glob('train2014/train2014/*.jpg')[:1] # 训练集图片

# 读取训练集图片
# tqdm用于显示进度条
for i in tqdm(range(len(paths))):
    path = paths[i]
    image = imread(path)  # 读取图片，返回一个numpy数组
...
print(X_data.shape)
vgg = scipy.io.loadmat('imagenet-vgg-verydeep-19.mat') # 加载vgg19模型
...
# style_index = 0 # 风格索引
X_style_data = resize_and_crop(imread(style_image), image_size)
...
style_name = mySettings.style_name
style_name = style_name[style_name.find('/') + 1:].rstrip('.jpg')
OUTPUT_DIR = 'samples_styles/%s' % style_name
...
X_sample = imread('content/%s'%mySettings.content_image)
